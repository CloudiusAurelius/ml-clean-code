...
logger = setup_logging(logfile='./logs/churn_library_tests.log')
...
def test_create_target(import_dataframe_fixture):
    ...
    try:
        ...
        logger.error(
            "Testing create_target: The target column 'churn' was not created")
        raise err
...
def test_create_target_dist_plot(import_dataframe_fixture):
    ...
    try:
        ...
        logger.error("Testing create_target_dist_plot: %s", err)
        raise err
    try:
        assert os.path.exists(test_path)
    except AssertionError as err:
        logger.error(
            "Testing create_target_dist_plot: The plot was not saved to the specified path")
        raise err
...
def test_create_bar_plot(import_dataframe_fixture):
    ...
    df = import_dataframe_fixture
    # Use the first categorical column for testing
    test_column = cat_columns[0]
    ...
    try:
        ...
        logger.error(
            "Testing create_bar_plot:\
            The column name is not a string: %s", test_column)
        raise err
    try:
        create_bar_plot(df, test_column, output_dir)
        logger.info("Testing create_bar_plot: SUCCESS")
    except Exception as err:
        logger.error("Testing create_bar_plot: %s", err)
        raise err
    try:
        assert os.path.exists(test_path)
    except AssertionError as err:
        logger.error(
            "Testing create_bar_plot: The plot was not saved to the specified path")
        raise err
...
def test_create_histogram(import_dataframe_fixture):
    ...
    df = import_dataframe_fixture
    # Use the first quantitative column for testing
    test_column = quant_columns[0]
    ...
    try:
        ...
        logger.error("Testing create_histogram: %s", err)
        raise err
    try:
        assert os.path.exists(test_path)
    except AssertionError as err:
        logger.error(
           	"Testing create_histogram: The plot was not saved to the specified path")
        raise err
...
def test_create_density_plot(import_dataframe_fixture):
    ...
    try:
        ...
        logger.error("Testing create_density_plot: %s", err)
        raise err
    try:
        assert os.path.exists(test_path)
    except AssertionError as err:
        logger.error(
            "Testing create_density_plot: The plot was not saved to the specified path")
        raise err
...
def test_create_heatmap(import_dataframe_fixture):
    ...
    try:
        ...
        logger.error("Testing create_heatmap: %s", err)
        raise err
    try:
        assert os.path.exists(test_path)
    except AssertionError as err:
        logger.error(
            "Testing create_heatmap: The heatmap was not saved to the specified path")
        raise err
...
def test_perform_eda(import_dataframe_fixture):
    ...
    try:
        ...
        logger.error("Testing perform_eda: %s", err)
        raise err
    try:
        assert success == 1
    except AssertionError as err:
        logger.error(
            "Testing perform_eda: The EDA function did not complete successfully")
        raise err
    try:
        ...
        for column in cat_columns:
            ...
            assert os.path.exists(
                test_path), f"File {filename} does not exist in {output_dir_eda}"
...
def test_encoder_helper(import_dataframe_fixture):
    ...
    try:
        ...
        logger.error("Testing encoder_helper: %s", err)
        raise err
    try:
...
def test_perform_feature_engineering(import_dataframe_fixture):
    ...
    try:
        ...
        logger.error("Testing perform_feature_engineering: %s", err)
        raise err
    try:
        assert len(X_train) > 0 and len(
            X_test) > 0, "Training and test sets are empty"
    except AssertionError as err:
        logger.error("Testing perform_feature_engineering: %s", err)
        raise err
    try:
        assert len(y_train) > 0 and len(
            y_test) > 0, "Training and test labels are empty"
    except AssertionError as err:
        logger.error("Testing perform_feature_engineering: %s", err)
        raise err
    try:
        assert X_train.shape[1] == len(
            features), "X_train does not have the correct number of features"
        assert X_test.shape[1] == X_train.shape[1],\
            "X_train and X_test have different number of features"
        assert y_train\
               .reshape(-1, 1)\
               .shape[1] == y_test.reshape(-1,1)\
               .shape[1], "y_train and y_test have different number of features"
    except AssertionError as err:
        logger.error("Testing perform_feature_engineering: %s", err)
        raise err
    try:
...
def test_classification_report_image(feature_engineering_fixture):
    ...
    try:
        ...
        logger.error("Testing classification_report_image: %s", err)
        raise err
    try:
        assert os.path.exists(
            test_path_rf), f"File {filename_rf} does not exist in {output_dir}"
    except AssertionError as err:
        logger.error("Testing classification_report_image: %s", err)
        raise err
...
def test_feature_importance_plot(feature_engineering_fixture):
    ...
    try:
        assert os.path.exists(
            test_path), f"File {filename} does not exist in {output_dir}"
    except AssertionError as err:
        logger.error("Testing feature_importance_plot: %s", err)
        raise err
...
def test_plot_roc_curve(feature_engineering_fixture):
    ...
    try:
        ...
        logger.error("Testing plot_roc_curve: %s", err)
        raise err
    try:
        assert os.path.exists(
            test_path), f"File {filename} does not exist in {output_dir}"
...
def test_shap_plot(feature_engineering_fixture):
    ...
    try:
        assert isinstance(
            features, list), "Features are not provided as a list"
    except AssertionError as err:
        logger.error("Testing shap_plot: %s", err)
        raise err
    try:
        assert len(
            features) == X_test.shape[1], "Number of features does not match the model input"
    except AssertionError as err:
        logger.error("Testing shap_plot: %s", err)
        raise err
    try:
...
def test_train_models(feature_engineering_fixture):
    ...
    try:
        ...
        assert os.path.exists(
            test_path_rf), f"File {filename_rf} does not exist in {output_dir_images_test}"

        filename_lr = 'lc_classification_report.png'
        test_path_lr = os.path.join(output_dir_images_test, filename_lr)
        assert os.path.exists(
            test_path_lr), f"File {filename_lr} does not exist in {output_dir_images_test}"

        filename_roc_curve = 'randomforestclassifier_roc_curve.png'
        test_path_roc_curve = os.path.join(
            output_dir_images_test, filename_roc_curve)
        assert os.path.exists(
            test_path_roc_curve),\
            f"File {filename_roc_curve} does not exist in {output_dir_images_test}"
        ...
        test_path_feature_importance = os.path.join(
            output_dir_images_test, filename_feature_importance)
        assert os.path.exists(
            test_path_feature_importance),\
            f"File {filename_feature_importance} does not exist in {output_dir_images_test}"

        filename_shap = 'shap_summary_plot_rf.png'
        test_path_shap = os.path.join(output_dir_images_test, filename_shap)
        assert os.path.exists(
            test_path_shap), f"File {filename_shap} does not exist in {output_dir_images_test}"

        # Check if models are saved
        filename_rc = 'rfc_model.pkl'
        path_rc = os.path.join(output_dir_models_test, filename_rc)
        assert os.path.exists(
            path_rc), f"File {filename_rc} does not exist in {output_dir_models_test}"

        filename_lr = 'logistic_model.pkl'
        path_lr = os.path.join(output_dir_models_test, filename_lr)
        assert os.path.exists(
            path_lr), f"File {filename_lr} does not exist in {output_dir_models_test}"
    except AssertionError as err:
        logger.error("Testing train_models: %s", err)
        raise err
